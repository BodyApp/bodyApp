...
angular.module('bodyAppApp')
  .controller('MainCtrl', function ($scope, $uibModal, $location, $http, $window, $state, $sce, $cookies, $interval, $timeout, Auth, User) {
    ...
    var powerWords = ['Confidence', 'Sexy', "Body", "Fitness", "Wellness", "Motivation", "Drive", "Success", "Strength", "Flexibility", "Community", "Bikini Body", "Lifestyle", "Good Habits", "Figure", "Attitude", "Naked", "Self", "Passion", "Inspiration", "Hotness", "Proof", "Glory", "Warrior", "Namaste", "Intensity", "Friends"];
    var indexNum = -1;
    pickPowerWord()
    ...
    function pickPowerWord() {
      $scope.selectedPowerWord = powerWords[++indexNum%powerWords.length]
      $timeout(function(){pickPowerWord()}, 5000)
    }

    // function pickPowerWord(indexNum) {
    //   // var selectedWord = powerWords[indexNum];
    //   // $scope.selectedPowerWord = powerWords[Math.floor(Math.random()*powerWords.length)];
      
    //   // $scope.selectedPowerWord = selectedWord
    //   $scope.selectedPowerWord = powerWords[indexNum]
